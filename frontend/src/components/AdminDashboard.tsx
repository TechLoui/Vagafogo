...
import {collection,query, where, getDocs, doc, deleteDoc,updateDoc, addDoc, setDoc, getDoc, arrayUnion } from 'firebase/firestore';
...
export default function AdminDashboard() {
  ...
  // Utilitário para exibir datas no formato brasileiro
          ...
          <section className="bg-white p-4 rounded shadow mb-6 w-full">
            ...
            <div className="mt-4 p-3 bg-gray-50 border rounded">
              <div className="flex flex-wrap items-end gap-3">
                ...
                <button onClick={fecharDiaSelecionado} className="px-3 py-1 bg-red-600 text-white rounded text-xs">
                  Fechar dia (todos pacotes)
                </button>
                <div className="flex items-center gap-2">
                  <select className="border px-2 py-1 rounded text-xs" value={horarioParaFechar} onChange={e => setHorarioParaFechar(e.target.value)}>
                    <option value="">Selecionar horário</option>
                    {horariosDisponiveis.map(h => (
                      <option key={h} value={h}>{h}</option>
                    ))}
                  </select>
                  <button onClick={fecharHorarioSelecionado} className="px-3 py-1 bg-orange-600 text-white rounded text-xs">
                    Fechar horário do dia
                  </button>
                </div>
                {horariosFechados.length > 0 && (
                  <div className="text-xs text-gray-700">Fechados: {horariosFechados.join(', ')}</div>
                )}
              </div>
            </div>
          </section>
