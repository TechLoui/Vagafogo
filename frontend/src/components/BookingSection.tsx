...
export function BookingSection() {
  ...
  // Buscar dias fechados para desabilitar no calendário
  useEffect(() => {
    (async () => {
      try {
        ...
        snap.docs.forEach((d) => {
          ...
          if (dados?.fecharDia) fechados.push(d.id);
        });
        setBlockedDates(fechados);
      } catch {
        setBlockedDates([]);
      }
    })();
    ...
    if (selectedDay) {
      ...
      if (isToday) {
        base = base.filter(h => {
          ...
          const ts = new Date(selectedDay.getFullYear(), selectedDay.getMonth(), selectedDay.getDate(), hh || 0, mm || 0).getTime();
          return ts > now.getTime();
        });
      }
    }
    if (horariosFechados?.length) {
      base = base.filter(h => !horariosFechados.includes(h));
    }
    return base;
  })();
  ...
  async function handleDate(date: Date | undefined) {
    setSelectedDay(date);
    setHorario("");
    setVagasRestantes(null);
    setHorariosFechados([]);
    if (date) {
      try {
        ...
        if (snap.exists()) {
          ...
          setHorariosFechados(Array.isArray(dados?.horariosFechados) ? dados.horariosFechados : []);
        }
      } catch {}
    }
